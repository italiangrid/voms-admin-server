...
def voms_lib_dir():
    ...
    return os.path.join(prefix, "usr", libdir, "voms")


class VOMSDefaults:
    db_props_template = os.path.join(template_prefix(), "database.properties")
    service_props_template = os.path.join(
        template_prefix(), "service.properties")
    ...
    voms_admin_war = os.path.join(
        voms_prefix(), "usr", "share", "webapps", "voms-admin.war")

    voms_admin_libs = glob.glob(os.path.join(
        voms_prefix(), "var", "lib", "voms-admin", "lib")+"/*.jar")
    voms_admin_classes = os.path.join(
        voms_prefix(), "var", "lib", "voms-admin", "tools")
    voms_admin_jar = os.path.join(
        voms_prefix(), "usr", "share", "java", "voms-admin.jar")

    voms_db_util = os.path.join(voms_prefix(), "usr", "sbin", "voms-db-util")
    voms_mysql_util = os.path.join(
        voms_prefix(), "usr", "sbin", "voms-mysql-util")
...
class X509Helper:
    ...
    def parse(self):
        ...
        # Handle also UID
        self.subject = re.sub(
            r'/(UserId|USERID|userId|userid|uid|Uid)=', '/UID=', self.subject)
