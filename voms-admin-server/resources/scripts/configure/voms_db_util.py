...
def setup_cl_options():
    parser.add_option(
        "--vo", dest="vo", help="the VO for which database operations are performed", metavar="VO")
    parser.add_option("--dn", dest="admin_dn",
                      help="the DN of the administrator certificate", metavar="DN")
    parser.add_option("--ca", dest="admin_ca",
                      help="the DN of the CA that issued the administrator certificate", metavar="DN")
    parser.add_option("--email", dest="admin_email",
                      help="the EMAIL address of the administrator", metavar="EMAIL")
    parser.add_option("--cert", dest="admin_cert",
                      help="the x.509 CERTIFICATE of the administrator being created", metavar="CERTIFICATE")
    parser.add_option("--ignore-cert-email", dest="admin_ignore_cert_email", action="store_true",
...
def check_args_and_options(options, args):
    ...
    if args[0] in ("add-admin", "remove-admin"):
        ...
        if (not options.admin_dn or not options.admin_ca) and not options.admin_cert:
            error_and_exit(
                "Please specify an administrator either providing a certificate with the --cert option, or with the --dn and --ca options.")
        ...
        if not options.admin_cert and options.admin_ignore_cert_email:
            error_and_exit(
                "The --ignore-cert-email must be used together with the --cert option.")
