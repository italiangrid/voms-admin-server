  ...
  <div  id="aclShowPane" style="position: relative;">
	...
	<s:if test="#attr.canModify">  	
  		<s:form action="change-reacceptance-period" namespace="/aup" cssStyle="clear:both; float: right">
      		...
      		<s:textfield name="period" value="%{model.reacceptancePeriod}" label="Acceptance period (in days)" size="4" labelposition="left"/>
        ...
        <s:if test="#attr.canModify">
          ...
          <s:if test="active">
          	<s:form action="trigger-acceptance" theme="simple">
          		<s:token/>
    			<s:submit value="%{'Trigger acceptance'}" onclick="openYesConfirmDialog(this, 'triggerReacceptanceDialog'); return false"/>      	
          	</s:form>
          </s:if>
          </s:if>
        </td>
