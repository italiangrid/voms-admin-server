...
<voms:hasPermissions var="canRead" context="vo" permission="r" />
  ...
  <div id="searchPane">
    <s:form validate="true" theme="simple" action="search">
      ...
      <s:if test="#attr.canReadPI">
        ...
        <s:checkbox id="limitToSuspendedCheck" name="limitToSuspendedUsers"
        ...
        <s:checkbox id="limitToAupCheck" name="limitToUsersWithPendingSignAUPRequest"
          onclick="$('#limitToSuspendedCheck').attr('checked', false); this.form.submit()" />
      </s:if>
      <s:else>
        <s:hidden name="limitToSuspendedUsers" value="%{'false'}" />
        <s:hidden name="limitToUsersWithPendingSignAUPRequest" value="%{'false'}" />
      </s:else>
      <s:label for="searchData.maxResults">Show:</s:label>
      <s:select name="searchData.maxResults" list="{'10','50','100'}" value="%{#session.searchData.maxResults}"
        onchange="this.form.submit()" />
    </s:form>
    <s:fielderror fieldName="searchData.text" />
  </div>
  <div id="createPane">
    <voms:hasPermissions var="canCreate" context="vo" permission="rw" />
        ...
        <table class="table">
          ...
          <tr class="userBulkOperations">
            <td style="border: none">
              <s:checkbox name="notSet" id="userSelectorTrigger"
                disabled="%{(not #attr.canCreate and not #attr.CanSuspend)}" theme="simple" />
            </td>
            <td colspan="3" style="border: none;">
              <s:submit value="%{'Suspend'}" align="right" action="bulk-suspend" theme="simple"
                cssClass="userActionButton" onclick=" return openSuspendDialog(this, 'suspendMultiUserDialog','');"
                disabled="%{#attr.canSuspend == false}" />
              <s:submit value="%{'Restore'}" align="right" action="bulk-restore" theme="simple"
                cssClass="userActionButton" disabled="%{#attr.canSuspend == false}" />
              <s:if
                test="not #attr.disableMembershipEndTime and #attr.canSuspend == true and #attr.readOnlyMembershipExpiration == false">
                <s:submit value="%{'Extend membership'}" align="right" action="bulk-extend-membership-expiration"
                  theme="simple" cssClass="userActionButton" disabled="%{#attr.canSuspend == false}"
              ...
              <s:submit value="%{'Delete'}" align="right" action="bulk-delete" theme="simple"
                cssClass="userActionButton" disabled="%{#attr.canCreate == false}"
          ...
          <s:iterator value="#session.searchResults.results" var="user" status="rowStatus">
            <tr class="tableRow">
              <td style="width: 1%; vertical-align: top; padding-top: .8em;">
                <s:checkbox name="userIds" fieldValue="%{id}" cssClass="userCheckbox" theme="simple" value="%{'false'}"
