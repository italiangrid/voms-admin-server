...
/***
 * The VOMS shutdown client. This client executes an HTTP GET for /shutdown on
 * localhost on a given port.
 */
public class ShutdownClient {
  ...
  public void doShutdown() {
    ...
    String shutdownEndpoint = String.format("http://localhost:%d/shutdown",
      port);
  ...
  static void usage() {
    ...
    formatter.printHelp(
      "java -cp ... org.glite.security.voms.admin.server.ShutdownClient",
      cliOptions);

  }

  /**
   * Creates a {@link ShutdownClient} starting from the properties specified in
   * a VO configuration
   * 
   * @param voName
   *          the name of the VO for which the {@link ShutdownClient} should be
   *          created
   * @return a {@link ShutdownClient} that can shut down the VO passed as
   *         argument
   */
  protected static ShutdownClient getShutdownClientForVO(String voName) {
    ...
    String password = conf
      .getString(VOMSConfigurationConstants.SHUTDOWN_PASSWORD);

    if (port == null) {

      System.err.format(
        "Shutdown port property not found in VO %s configuration!", voName);
      System.exit(1);
    }
  ...
  protected static int checkPortNumber(String portNumberString) {
    ...
    try {
      ...
      if (thePort > 65536 || thePort < 0) {
        ...
        System.err
          .print("Please provide a valid shutdown port! (0 < port < 65536)");
        System.exit(1);
      }
      ...
      System.err
        .print("Error parsing the shutdown port number (not an integer). "
          + e.getMessage());
      System.exit(-1);
    }
