...
/**
 * This service defines methods to manage certificates linked to a VOMS
 * membership.
 * 
...
public interface VOMSCertificates {
  ...
  /**
   * Adds a certificate to a VOMS membership, identified by an X.509
   * certificate.
   * 
   ...
   *          The VOMS user id.
   * 
   * @return An array of {@link X509Certificate} objects representing the
   *         certificates registered for the user
   */
  public X509Certificate[] getCertificates(long userId);

  /**
   * Returns the certificates currently bound to a given VOMS user, identified
  ...
  /**
   * Suspends a VOMS user certificate for a given reason. Suspended certificates
   * cannot be used to obtain VOMS attributes.
   * 
   * @param cert
  ...
  /**
   * Restores a suspended VOMS user certificate. Suspended certificates cannot
   * be used to obtain VOMS attributes.
   * 
   * @param cert
