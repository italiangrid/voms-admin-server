...
public class TestTimeIntervalNotifications {
  ...
  public void test() throws InterruptedException {
    ...
    TimeIntervalNotificationStrategy s = new TimeIntervalNotificationStrategy(
      new InMemoryNotificationTimeStorage(), new MockNotificationService(), 10,
      TimeUnit.SECONDS);
  ...
  public void testPeriod() throws InterruptedException {
    ...
    TimeIntervalNotificationStrategy s = new TimeIntervalNotificationStrategy(
      new InMemoryNotificationTimeStorage(), new MockNotificationService(), 1,
      TimeUnit.SECONDS);
  ...
  public void testPeriodicWarnings() throws InterruptedException {
    ...
    WarningsBeforeExpirationNotificationStrategy s = new WarningsBeforeExpirationNotificationStrategy(
      new InMemoryNotificationTimeStorage(), new MockNotificationService(),
      new int[] { 10, 5, 1 }, TimeUnit.SECONDS, startPlus30secs);
    ...
    for (int i = 0; i < expectedTimes.length; i++) {
      ...
      String msg = String.format("Notification dates do not match. "
        ...
        actualTime, Math.abs(expectedTimes[i] - actualTime));

      Assert.assertTrue(msg,
        timeEquals(expectedTimes[i], actualTime, TimeUnit.SECONDS.toMillis(1)));
    }
