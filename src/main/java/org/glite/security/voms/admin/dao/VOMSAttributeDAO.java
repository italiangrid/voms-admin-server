...
public class VOMSAttributeDAO {
    ...
    public VOMSAttributeDescription deleteAttributeDescription( String name ) {
        ...
        String attributeValueEntities[] = new String[]{
                "VOMSUserAttribute",
                "VOMSGroupAttribute",
                "VOMSRoleAttribute"
        };
        ...
        for (int i=0; i < attributeValueEntities.length; i++){
            String query = "delete from org.glite.security.voms.admin.model."+attributeValueEntities[i]+" where attributeDescription = :desc";
            HibernateFactory.getSession().createQuery( query ).setEntity( "desc", attrDesc).executeUpdate();
    ...
    public List getGroupAttributes(VOMSAttributeDescription desc){
    	...
    	return HibernateFactory.getSession().createQuery(query).setEntity("desc", desc).list();
    }
    ...
    public boolean isAttributeValueAlreadyAssigned(VOMSUser u, VOMSAttributeDescription desc, String attrValue){
        ...
        // Need to perform the check in memory since oracle has a bug (or I did not understand how to manage clob equality tests).
        Iterator i = q.iterate();
        while (i.hasNext()){
            String value = (String) i.next();
            
            // NULL attribute value can be assigned to multiple users, two NULLs aren't equal as attribute values 
            if (value == null)
                return false;
