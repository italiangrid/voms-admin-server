...
public class DeleteACLEntryOperation extends BaseVomsOperation {
    ...
    protected Object doExecute() {
        ...
        if (isRecursive()){
        	...
        	if (acl.getContext().isGroupContext()){
        		try{
        			...
        			while (childIter.hasNext()){
        				...
        				DeleteACLEntryOperation op = instance(childGroup.getACL(), admin, recursive);
        				op.execute();
        			}
                
                
        			log.debug("Removing ACL entry for admin '"+admin+"' in ACL '"+acl.getContext()+"' [recursive].");
        	...
        	log.debug("Removing ACL entry for admin '"+admin+"' in ACL '"+acl.getContext()+"'.");
        	ACLDAO.instance().deleteACLEntry( acl, admin );
        	return acl;
        }
