...
public class CreateUserAction extends BaseAction {
	...
	public ActionForward execute(
		...
		log.debug(ToStringBuilder.reflectionToString(uForm));
		
		VOMSUser usr = (VOMSUser) CreateUserOperation.instance(uForm).execute();
		
		String certType = uForm.getCertificateType();
		
		if ("dnCa".equals(certType)){
			
			VOMSUserDAO.instance().addCertificate(usr, uForm.getDn(), uForm.getCa());
			
		}
		
		if ("cert".equals(certType)){
			
			if (uForm.getCertificateFile() == null)
				throw new NullArgumentException("Certificate to be added is null!");
			
			X509Certificate cert = CertUtil.parseCertficate(uForm.getCertificateFile().getInputStream());
			
			VOMSUserDAO.instance().addCertificate(usr, cert);
		}

		request.setAttribute("newUser", usr);
