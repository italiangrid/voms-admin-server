...
public class PanelTag extends TagSupport {
    ...
    protected String getShowJs(){
        if (isActive())
            return "show('"+getUrl()+"','"+getContentId()+"',this,'"+getImgUrl()+"')";
        else
            return "hide('"+getUrl()+"','"+getContentId()+"',this,'"+getImgUrl()+"')";
    }
    
    
    protected String getJs(){
        
        return "toggleVisibility('"+getUrl()+"', '"+getId()+"','"+getImgUrl()+"')";
    }
    ...
    protected void buildHeaderContent(Document doc){
        ...
        buttonElement.setAttribute("class",(buttonClass==null?"":buttonClass));
    ...
    public void write() throws JspException{
        ...
        // Write panel element
        tUtils.write(pageContext,"<div id=\""+getId()+"\" class='"+getPanelClass()+"' >");
        
        // Use dom to build quite complicated (and self contained) header element
        ...
        // Write header
        tUtils.write(pageContext,headerString);
        
        // Write content
        // 
                
        if (isActive())
        		tUtils.write(pageContext,"<div id=\""+getContentId()+"\" class='"+(getContentClass()==null?"":getContentClass())+"' style='clear:both'>");
        else
        		tUtils.write(pageContext,"<div id=\""+getContentId()+"\" class='"+(getContentClass()==null?"":getContentClass())+"' style='clear: both; display: none'>");
        
        tUtils.write(pageContext,"<div class='separator'>&nbsp;</div>");
    ...
    public int doEndTag() throws JspException {
        ...
        TagUtils tUtils = TagUtils.getInstance();
        tUtils.write(pageContext,"</div>");  
        tUtils.write(pageContext,"</div><!-- "+getContentId()+" --> </div> <!-- "+getId()+" -->");
        tUtils.write(pageContext,"<div class='separator'>&nbsp;</div>");
        return EVAL_PAGE;
    }
