...
public class BaseSearchOperation extends BaseVoReadOperation implements SearchOperation{

	SearchData searchData;
	
	int userMaxResults  = VOMSConfiguration.instance().getInt(VOMSConfiguration.USER_MAX_RESULTS_PER_PAGE, 15);
	int groupMaxResults = VOMSConfiguration.instance().getInt(VOMSConfiguration.GROUP_MAX_RESULTS_PER_PAGE, 15);
	int roleMaxResults = VOMSConfiguration.instance().getInt(VOMSConfiguration.ROLE_MAX_RESULTS_PER_PAGE, 15);
	int attributeMaxResults = VOMSConfiguration.instance().getInt(VOMSConfiguration.ATTRIBUTES_MAX_RESULTS_PER_PAGE, 15);
	...
	protected Object doExecute() {
		...
		if (searchData == null)
			...
			if (searchType.equals("user"))
				...
				return SearchGroupsOperation.instance(searchData.getText(), searchData.getFirstResult(), groupMaxResults).execute();
