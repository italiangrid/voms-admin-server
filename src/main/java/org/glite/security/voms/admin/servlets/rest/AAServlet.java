...
public class AAServlet extends ACServlet {
	...
	protected void writeErrorResponse(HttpServletResponse response, int httpErrorCode,
			String vomsErrorCode, String errorMessage) {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		if (CurrentAdmin.instance().isUnauthenticated()) {
			writeErrorResponse(response, 400, VOMS_ERROR_BAD_REQUEST,
		...
		try {
			...
			writeErrorResponse(response, 400, VOMS_ERROR_BAD_REQUEST,
					"The client is not authorized to request VOMS attributes in this VO");
			return;
			...
			log.error("Error getting VOMS attributes for user '" + voMember + "':" + e.getMessage());
			...
			writeErrorResponse(response, 500, VOMS_ERROR_INTERNAL_ERROR, "Internal server error: "
	...
	private void writeResponse(HttpServletResponse response, VOMSAttributes attrs) {
		...
		try {
			...
			response.getWriter().write(StringUtils.join(attrs.getFqans(), "\n"));
			response.getWriter().write("\n");
			response.getWriter().write(StringUtils.join(attrs.getGenericAttributes(), "\n"));
