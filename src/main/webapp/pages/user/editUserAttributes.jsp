	...
	<c:choose>
		<c:when test="${!empty attributeDescriptions}">
			<div class="attributesCreationPane">
				<html:form action="/UserAttributes" method="post" onsubmit="return validateUserAttributesForm(this)">
					...
					<table class="form" cellpadding="0" cellspacing="0">
						...
						<tr>
							<td/>
							<td>
								<voms:submit context="vo" permission="ATTRIBUTES_WRITE" styleClass="submitButton" value="Set an attribute"/>				
							</td>
						</tr>				
					</table>
					...
					<c:if test="${fn:length(vomsUser.attributes) > 0}">
						...
						<c:forEach var="attribute" items="${vomsUser.attributes}" varStatus="status">
							...
							<tr class="${ (status.index) %2 eq 0 ? 'tableRowEven': 'tableRowOdd'}">
								...
								<td class="tableCell">
									<div class="actions">
										<c:url value="/UserAttributes.do" var="deleteUserAttributeURL">
...
<html:javascript formName="/UserAttributes"/>